...
# ── 5.5b Unit test: draw_status_box() ─────────────────────────────────────


class TestDrawStatusBox(unittest.TestCase):
    """Verify draw_status_box() renders correct layout and width."""

    @patch("main.shutil.get_terminal_size")
    def test_status_box_layout_en(self, mock_term_size):
        mock_term_size.return_value.columns = 80
        state = {
            "hp": 100,
            "oro": 20,
        ...
        result = draw_status_box(state, "en")
        
        # Check border and content lines
        self.assertIn("╔════════════════════════════════════════════╗", result)
        self.assertIn("║  ◆  KAREL: BYTEBOUND  ◆                    ║", result)
        self.assertIn("║  HP  ████████████████████  100/100         ║", result)
        self.assertIn("║  Gold  20            Turn  0               ║", result)
        self.assertIn("║  Bag   (empty)                             ║", result)
        self.assertIn("╚════════════════════════════════════════════╝", result)
        
        # Check that all lines are exactly 46 chars long (including borders)
        lines = result.split("\n")
        for line in lines:
            self.assertEqual(len(line), 46)

    @patch("main.shutil.get_terminal_size")
    def test_status_box_layout_es(self, mock_term_size):
        mock_term_size.return_value.columns = 80
        state = {
            "hp": 85,
            "oro": 15,
        ...
        result = draw_status_box(state, "es")
        
        self.assertIn("╔════════════════════════════════════════════╗", result)
        self.assertIn("║  ◆  KAREL: BYTEBOUND  ◆                    ║", result)
        self.assertIn("║  HP  █████████████████░░░  85/100          ║", result)
        self.assertIn("║  Oro  15             Turno  3              ║", result)
        self.assertIn("║  Mochila   Café                            ║", result)
        self.assertIn("╚════════════════════════════════════════════╝", result)
    ...
    @patch("main.shutil.get_terminal_size")
    def test_status_box_narrow_terminal(self, mock_term_size):
        mock_term_size.return_value.columns = 40
        state = {
            "hp": 100,
            "oro": 20,
...
class TestGameLoopNoCrash(unittest.TestCase):
    ...
    @patch("main.screen_break")
    @patch("main.call_gpt")
    @patch("time.sleep", return_value=None)  # speed up typewriter
    @patch("builtins.input")
    def test_game_loop_exit_immediately(self, mock_input, mock_sleep, mock_gpt, mock_screen_break):
    ...
    @patch("builtins.input")
    def test_game_loop_one_turn_victory(self, mock_input, mock_sleep, mock_gpt, mock_screen_break):
    ...
    @patch("time.sleep", return_value=None)
    @patch("builtins.input")
    def test_game_over_by_hp(self, mock_input, mock_sleep, mock_gpt, mock_screen_break):
    ...
    @patch("time.sleep", return_value=None)
    @patch("builtins.input")
    def test_invalid_backpack_index_no_crash(self, mock_input, mock_sleep, mock_gpt, mock_screen_break):
        """Type non-numeric option when backpack is not empty — should not crash with IndexError."""
        mock_input.side_effect = ["1", "1", "no lo se wei", "exit"]
        mock_gpt.return_value = json.dumps({
            "narrativa": "You found a potion.",
            "opciones": ["Keep going"],
            "cambio_vida": 0,
            "item_encontrado": "Potion",
        })
        from main import main
